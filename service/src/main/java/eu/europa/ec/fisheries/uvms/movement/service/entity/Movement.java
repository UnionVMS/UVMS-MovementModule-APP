...
@NamedQueries({
    ...
    @NamedQuery(name = Movement.FIND_PREVIOUS, query = "SELECT m FROM Movement m  WHERE  m.timestamp = (select max(mm.timestamp) from Movement mm where mm.movementConnect.id = :id and mm.timestamp < :date and mm.movementSource in :sources) AND m.movementConnect.id = :id "),
