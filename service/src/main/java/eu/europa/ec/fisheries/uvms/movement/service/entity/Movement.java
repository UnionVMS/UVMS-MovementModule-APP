...
@NamedQueries({
    ...
    @NamedQuery(name = MicroMovementExtended.FIND_ALL_BETWEEN_DATES_FOR_CONNECT_IDS, query = "SELECT new eu.europa.ec.fisheries.uvms.movement.service.dto.MicroMovementExtended(m.location, m.heading, m.id, m.movementConnect.id, m.timestamp, m.speed, m.movementSource) FROM Movement m WHERE m.timestamp >= :fromDate AND m.timestamp <= :toDate AND m.movementConnect.id in :connectIds ORDER BY m.timestamp DESC"),
