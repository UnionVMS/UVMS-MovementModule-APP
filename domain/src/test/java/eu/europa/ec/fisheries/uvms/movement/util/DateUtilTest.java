...
public class DateUtilTest extends TransactionalTests {
	...
	public void testGetDateFromString() throws ParseException {
		...
		OffsetDateTime testDate = OffsetDateTime.of(2018, 3, 9, 9, 26, 30, 0, ZoneOffset.ofHours(2));
		testDate = testDate.withOffsetSameInstant(ZoneOffset.UTC);
		...
		OffsetDateTime timestamp = DateUtil.getDateFromString("2018-03-09 10:26:30 +0100");
		assertTrue(testDate.isEqual(timestamp));
	...
	public void testParseToUTCDate() throws ParseException { //To UTC Date is somewhat missleading, the function simply parses a string into a date. 
		...
		//Formats are in DateFormats.java
		//yyyy-MM-dd HH:mm:ss Z      					  2018-03-09 09:26:30 +0100
		OffsetDateTime timestamp = DateUtil.parseToUTCDate("2018-03-09 10:26:30 +0100");
		assertTrue(testDate.isEqual( timestamp));
	...
	public void testParseUTCDateToString() {
		...
		testDate = OffsetDateTime.of(2018,3, 9, 3, 26, 30, 00, ZoneOffset.ofHours(2));
		ZonedDateTime zonedTestDate = ZonedDateTime.of(2018,3, 9, 3, 26, 30, 00, ZoneId.of("CEST"));
		zonedTestDate = zonedTestDate.withZoneSameInstant(ZoneId.of("CST"));

		formatedDate = DateUtil.parseUTCDateToString(zonedTestDate.toOffsetDateTime());
		assertTrue(formatedDate.contentEquals("2018-03-09 09:26:30 +0000"));
	...
	public void testAddSecondsToDateWithXMLGregorianCalendarInput() throws DatatypeConfigurationException {
		OffsetDateTime testDate = OffsetDateTime.of(2018, 3, 9, 9, 26, 59, 0, ZoneOffset.ofHours(2));
		testDate = testDate.withOffsetSameInstant(ZoneOffset.UTC);


		OffsetDateTime output = DateUtil.addSecondsToDate(testDate, 1);
		assertEquals(testDate.plusSeconds(1), output);
		
		testDate = testDate = OffsetDateTime.of(2018, 3, 9, 9, 26, 32, 0, ZoneOffset.ofHours(2));
		output = DateUtil.addSecondsToDate(testDate, 52);
		assertEquals(testDate.plusSeconds(52), output);
		
		OffsetDateTime n = null;
		try {
	...
	public void testAddSecondsToDateWithDateInput() throws DatatypeConfigurationException { //Almost a carbon copy of the one above
		...
		OffsetDateTime output = DateUtil.addSecondsToDate(testDate, 0);
		assertEquals(testDate.plusSeconds(1), output);

		testDate = OffsetDateTime.of(2018, 3, 9, 9, 26, 32, 0, ZoneOffset.ofHours(2));
		output = DateUtil.addSecondsToDate(testDate, 52);
		assertEquals(testDate.plusSeconds(52), output);
	...
	public void testNowUTC() {
		...
		OffsetDateTime output = DateUtil.nowUTC();
